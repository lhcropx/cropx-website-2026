...
function cropx_render_customer_stories_grid( $atts = array() ) {
	...
	<section class="pa-grid-section" aria-label="<?php esc_attr_e( 'Results & Research', 'cropx' ); ?>">
		...
		<div class="pa-grid-header pa-grid-header--dual-pills">
			...
			// to occupy. Hidden sitewide (Aug 2026) until Story Tags get organized
			// into a cleaner taxonomy — flip $show_topic_tags back to true to
			// restore this row; nothing else here needs to change. The taxonomy,
			// terms, and query logic above are untouched, so /results/tag/{slug}/
			// URLs (if visited directly) still work — they're just not linked from
			// a pill anymore in the meantime.
			...
			if ( $show_topic_tags && ! is_wp_error( $story_tags ) && ! empty( $story_tags ) ) :
			?>
