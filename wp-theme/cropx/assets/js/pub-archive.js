...
( function () {
	...
	if ( grid && btn ) {
		...
		function buildCard( pub ) {
			...
			const rawExcerpt = ( pub.cropx_card_excerpt || '' ).trim();
			if ( rawExcerpt ) {
		...
		// reaching the sentinel again right as new cards land). The click
		// handler already got this for free from btn.disabled, but the
		// auto-trigger below needs its own check since it doesn't go through
		// a disabled button.
		let isLoading = false;
		...
		// feels like a seamless infinite-scroll grid rather than a real "wait
		// for it to pop in" moment. Falls back to leaving the manual button as
		// the only way to load more if IntersectionObserver isn't supported.
		let observer = null;
		if ( showMoreWrap && 'IntersectionObserver' in window ) {
