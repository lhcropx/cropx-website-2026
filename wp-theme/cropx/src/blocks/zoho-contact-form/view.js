...
initScrollReveal( '.zcf-section' );

document.querySelectorAll( '.zcf-form' ).forEach( ( form ) => {
	const submitBtn      = form.querySelector( '.zcf-submit' );
	const statusEl       = form.querySelector( '.zcf-status' );
	const countryField   = form.querySelector( '.zcf-country-select' );
	const stateCityRow   = form.querySelector( '.zcf-row--state-city' );
	const stateField     = form.querySelector( '.zcf-state-select' );
	const stateFieldWrap = form.querySelector( '.zcf-field--state' );
	const cityFieldWrap  = form.querySelector( '.zcf-field--city' );
	...
	function syncGeoFields() {
		...
		if ( stateCityRow ) {
			...
			stateCityRow.classList.toggle( 'zcf-row--single', showState && ! showCity );
		}
	...
	function setFieldError( field, message ) {
		const wrapper = field.closest( '.zcf-field' );
		if ( ! wrapper ) return;
		wrapper.classList.add( 'has-error' );

		let errorEl = wrapper.querySelector( '.zcf-field-error' );
		if ( ! errorEl ) {
			errorEl = document.createElement( 'span' );
			errorEl.className = 'zcf-field-error';
	...
	function clearFieldError( field ) {
		...
		const errorEl = wrapper.querySelector( '.zcf-field-error' );
	...
	function showStatus( message, type ) {
		...
		statusEl.className   = `zcf-status zcf-status--${ type }`;
