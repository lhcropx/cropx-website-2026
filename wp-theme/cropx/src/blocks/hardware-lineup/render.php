...
$hwf_wrapper_extra_attrs = [ 'class' => 'hwf-section reveal-group hwf-section--bg-' . $bg_color, 'data-section-bg' => $bg_color ];
...
<section <?php echo $wrapper_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	...
	<div class="hwf-header">
		<?php if ( $eyebrow ) : ?>
			<p class="section-eyebrow reveal-up" style="color: var(--<?php echo $eyebrow_color; ?>); --reveal-delay:0.05s"><?php echo esc_html( $eyebrow ); ?></p>
		<?php endif; ?>
	...
	>
		<div class="hwc-track">
			<?php
			// Staggered per-item reveal delay — same convention as Product
			// Grid, starting after the header eyebrow's own 0.05s delay.
			$hwf_reveal_index = 0;
			foreach ( $items as $item ) :
				...
				$hwf_reveal_delay = 0.15 + ( min( $hwf_reveal_index, 8 ) * 0.06 );
				$hwf_reveal_index++;
				$pill_style  = '--hwf-img-h: ' . $img_h . '%; --reveal-delay:' . $hwf_reveal_delay . 's';
			?>
				<a
					class="hwf-pill reveal-item"
