...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className:
			`sci-section sci-section--${ backgroundVariant }` +
			( ! isBlue && segmentAccent !== 'general'
				? ` sci-segment-${ segmentAccent }`
				: '' ),
		style: isBlue
			? { '--sci-pattern-url': `url(${ window.cropxThemeData?.themeUri ?? '' }assets/decorative/drift-pattern.svg)` }
			: undefined,
	} );
			...
			<section { ...blockProps }>
				<div className="sci-inner">
					...
					<div className="sci-grid">
						{ columns.map( ( col, idx ) => (
							<div key={ idx } className="sci-item">
								{ showIcons !== false && (
									<div className="sci-icon" aria-hidden="true">
										<img src={ iconSrc( col.icon ) } alt="" width="24" height="24" />
									</div>
								) }
								<RichText
									tagName="h3"
									className="sci-item-heading"
								...
								/>
								{ col.ctaLabel && (
									<span className="sci-cta sci-cta-preview" aria-hidden="true">
										{ col.ctaLabel }
										{ ARROW }
									</span>
								) }
