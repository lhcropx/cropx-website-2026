...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( { className: 'footer-brand' } );
		...
		<>
			<InspectorControls>
				...
				<PanelBody title={ __( 'Social links', 'cropx' ) } initialOpen={ false }>
					<TextControl label={ __( 'LinkedIn URL', 'cropx' ) }   value={ linkedinUrl }   onChange={ ( v ) => setAttributes( { linkedinUrl: v } ) } />
					<TextControl label={ __( 'X (Twitter) URL', 'cropx' ) } value={ xUrl }          onChange={ ( v ) => setAttributes( { xUrl: v } ) } />
					<TextControl label={ __( 'YouTube URL', 'cropx' ) }    value={ youtubeUrl }    onChange={ ( v ) => setAttributes( { youtubeUrl: v } ) } />
					<TextControl label={ __( 'Facebook URL', 'cropx' ) }   value={ facebookUrl }   onChange={ ( v ) => setAttributes( { facebookUrl: v } ) } />
					<TextControl label={ __( 'Instagram URL', 'cropx' ) }  value={ instagramUrl }  onChange={ ( v ) => setAttributes( { instagramUrl: v } ) } />
				</PanelBody>
			...
			<div { ...blockProps }>
				...
				<div className="footer-contact">
					...
					{ showLocations !== false && (
						<a href="#">
							<span className="footer-locations-text">{ locationsText ?? 'Anaheim · Melbourne · Wellington · Haren · Netanya' }</span>
						</a>
					) }
				</div>
