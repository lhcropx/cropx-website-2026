...
/*
 * Content Card Carousel — editor UI.
 *
 * This is deliberately a close port of the Cards block's edit.js (same three
 * content-source modes: manual / posts / auto-single / auto-mixed, same
 * reorder/drag mechanics, same post-picker and taxonomy-filter panels) —
 * see src/blocks/cards/edit.js for the canonical write-up of each control.
 * The two things that are actually different here: (1) every card gains an
 * optional statValue/statLabel pair for the photo overlay badge (manual and
 * Choose Posts modes only — see render.php for why Auto mode never shows
 * one), and (2) the canvas renders cards stacked full-width in the editor
 * (.cc-editor-stack) rather than the scroll-snap track — RichText fields
 * inside a horizontally-scrolling flex track are awkward to reach/edit in a
 * narrow iframe, and the front end's real carousel behavior isn't something
 * an editor needs to preview by scrolling sideways in the canvas anyway.
 */
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const tagClass = isDark ? 'cc-tag cc-tag--white' : 'cc-tag cc-tag--dark';

	const blockProps = useBlockProps( {
		className: `cc-section cc-section--bg-${ bgColor }`,
		style: bgColor === 'deep-blue'
			? { '--cc-pattern-url': `url(${ window.cropxThemeData?.themeUri ?? '' }assets/decorative/drift-pattern.svg)` }
			: undefined,
	} );
	...
	// ── Resolve each manual card's photo fresh from its attachment ID —
	// same rationale as Cards block's edit.js (stale photoUrl snapshots). ──
	...
	function addSlot() {
		setAttributes( {
			manualPosts: [ ...slots, { postId: 0, titleOverride: '', excerptOverride: '', imageId: 0, imageUrl: '', imageAlt: '', ctaLabel: '', statValue: '', statLabel: '' } ],
		} );
	}
	...
	function PreviewCard( { title, rawExcerpt, imageUrl, ctaLabel = 'Read more', statValue = '', statLabel = '' } ) {
		const noTitle = ! title;
		return (
			<article className={ `cc-card${ isDark ? ' cc-card--dark' : '' }` }>
				<div className="cc-card-photo" style={ { position: 'relative' } }>
					{ imageUrl ? (
						<img className="cc-card-img" src={ imageUrl } alt="" />
					) : (
						<div style={ {
							width: '100%', height: '100%', minHeight: '12rem',
					...
					{ ( statValue || statLabel ) && (
						<div className="cc-stat-badge">
							{ statValue && <span className="cc-stat-value">{ statValue }</span> }
							{ statLabel && <span className="cc-stat-label">{ statLabel }</span> }
						</div>
					) }
				</div>
				<div className="cc-card-body">
					<h3 className="cc-title" style={ { fontStyle: noTitle ? 'italic' : 'normal', opacity: noTitle ? 0.4 : 1 } }>
						{ noTitle ? __( '— No post selected —', 'cropx' ) : title }
					</h3>
					{ showExcerpt && rawExcerpt && (
						<p
							className={ `cc-excerpt cc-excerpt--lines-${ excerptLines }` }
							/* eslint-disable-next-line react/no-danger */
							dangerouslySetInnerHTML={ { __html: rawExcerpt } }
						/>
					) }
					<span className="cc-cta" aria-hidden="true">
						{ ctaLabel } <Arrow />
					</span>
				</div>
			</article>
		...
		<>
			<InspectorControls>
				...
				<PanelBody title={ __( 'Section Settings', 'cropx' ) } initialOpen={ true }>
					...
					<ToggleControl
						...
						help={ __( 'Automatically scrolls to the next card every few seconds. Pauses on hover and while a visitor is interacting with the carousel.', 'cropx' ) }
						checked={ !! autoAdvance }
						...
						{ cards.map( ( card, idx ) => (
							...
							>
								...
								<p style={ { fontSize: '11px', color: '#757575', margin: '10px 0 4px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' } }>
									{ __( 'Stat badge (optional)', 'cropx' ) }
								</p>
								<TextControl label={ __( 'Stat value', 'cropx' ) } placeholder={ __( 'e.g. $12M', 'cropx' ) } value={ card.statValue ?? '' } onChange={ ( v ) => updateCard( idx, 'statValue', v ) } />
								<TextControl label={ __( 'Stat label', 'cropx' ) } placeholder={ __( 'e.g. in credit secured', 'cropx' ) } value={ card.statLabel ?? '' } onChange={ ( v ) => updateCard( idx, 'statLabel', v ) } />
								<Button onClick={ () => removeCard( idx ) } variant="link" isDestructive disabled={ cards.length <= 1 } style={ { marginTop: '8px' } }>
							...
							>
								...
								{ slot.postId > 0 && (
									<>
										...
										<TextControl label={ __( 'Stat value', 'cropx' ) } placeholder={ __( 'e.g. $12M', 'cropx' ) } value={ slot.statValue ?? '' } onChange={ ( v ) => updateSlot( idx, 'statValue', v ) } />
										<TextControl label={ __( 'Stat label', 'cropx' ) } placeholder={ __( 'e.g. in credit secured', 'cropx' ) } value={ slot.statLabel ?? '' } onChange={ ( v ) => updateSlot( idx, 'statLabel', v ) } />
									</>
								) }
								<Button onClick={ () => removeSlot( idx ) } variant="link" isDestructive disabled={ slots.length <= 1 } style={ { marginTop: '8px' } }>
			...
			<section { ...blockProps }>
				<div className="cc-inner">
					...
					{ /* ── Manual mode canvas ── */ }
					{ queryMode === 'manual' && (
						<div className="cc-editor-stack">
							{ cards.map( ( card, idx ) => {
								const resolvedPhotoUrl = resolveCardPhotoUrl( card, idx );
								return (
								<article key={ idx } className={ `cc-card${ isDark ? ' cc-card--dark' : '' }` }>
									<div className="cc-card-photo">
										{ resolvedPhotoUrl ? (
											<>
												<img className="cc-card-img" src={ resolvedPhotoUrl } alt={ card.photoAlt } />
												{ ( card.statValue || card.statLabel ) && (
													<div className="cc-stat-badge">
														{ card.statValue && <span className="cc-stat-value">{ card.statValue }</span> }
														{ card.statLabel && <span className="cc-stat-label">{ card.statLabel }</span> }
													</div>
												) }
											</>
										) : (
									...
									<div className="cc-card-body">
										{ ( card.tag || card.date ) && (
											<div className="cc-meta">
												...
												{ card.date && <span className={ isDark ? 'cc-date cc-date--dark' : 'cc-date' }>{ card.date }</span> }
											</div>
										) }
										<RichText
											tagName="h3"
