...
Full state of the CropX website rebuild as of **July 25, 2026**. Use this as a context primer for any new Claude session so we never lose progress.
...
**Today (July 25, 2026):** Custom permalink structure for the `cropx_publication` CPT — single publications now live at `/results/{content-type}/{post-name}/` instead of the flat `/results/{post-name}/`. Blog post permalinks also restructured (by hand, in Settings → Permalinks — no code) to nest under `/blog/` instead of the site root. See "Done since last update" below.
