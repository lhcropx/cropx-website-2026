 ...
 *
 * Items are stored in a flat `columns` array attribute and rendered in order
 * into a standard CSS Grid (see .ici-columns in style.css) — the browser's
 * native grid auto-flow places them left-to-right, wrapping to a new row
 * once the current row fills, exactly like reading order. No manual
 * distribution math is needed here; $column_count only picks which
 * grid-template-columns class (.ici-cols-4/5/6) is applied.
 *
 * Attributes:
