 ...
 *
 * Two-panel card grid (CropX Platform / CropX Hardware). Editors switch
 * between panels via the canvas tab bar or the Items panel tab switcher.
 * Each item uses the full product-grid editing interface: photo with focal
 * point + zoom, and illustration overlay controls (card-bleed or contained).
 *
...
export default function Edit( { attributes, setAttributes } ) {
	...
	useEffect( () => {
		...
		// Walk up to find the sidebar's scrollable container.
		...
		// Offset below the sticky "Page | Block" tabs bar.
		...
		<>
			...
			<InspectorControls>
				...
				<PanelBody title={ __( 'Tab labels', 'cropx' ) } initialOpen={ false }>
					<TextControl
						label={ __( 'Platform tab', 'cropx' ) }
						value={ platformLabel }
						onChange={ ( v ) => setAttributes( { platformLabel: v } ) }
					/>
					<TextControl
						label={ __( 'Hardware tab', 'cropx' ) }
						value={ hardwareLabel }
						onChange={ ( v ) => setAttributes( { hardwareLabel: v } ) }
					/>
				</PanelBody>

				<PanelBody title={ __( 'Platform panel header', 'cropx' ) } initialOpen={ false }>
					<TextControl
				...
				<PanelBody title={ __( 'Hardware panel header', 'cropx' ) } initialOpen={ false }>
					<TextControl
						label={ __( 'Eyebrow', 'cropx' ) }
				...
				<PanelBody title={ __( 'Items', 'cropx' ) } initialOpen={ true }>
					...
					{ /* Tab switcher — select which panel's items to edit */ }
					<div className="ptabs-items-switcher">
						<button
							type="button"
							className={ `ptabs-items-switcher__btn${ isPlatform ? ' is-active' : '' }` }
			...
			<div { ...blockProps }>
				...
				{ /* Tab bar — functional so editors can switch panels on canvas */ }
				<div className="ptabs-editor-tab-bar">
					<button
						type="button"
						className={ `ptabs-editor-tab${ isPlatform ? ' is-active' : '' }` }
						onClick={ () => switchTab( 'platform' ) }
					>
						{ platformLabel }
					</button>
