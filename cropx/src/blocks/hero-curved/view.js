 ...
 * block that immediately follows the hero and sets --hc-swoop-fill so the SVG
...
document.addEventListener( 'DOMContentLoaded', () => {
	document.querySelectorAll( '.wp-block-cropx-hero-curved' ).forEach( ( block ) => {
		...
		if ( bg && bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent' ) {
			const wrap = block.querySelector( '.hc-bleed-wrap' );
			if ( wrap ) {
				wrap.style.setProperty( '--hc-swoop-fill', bg );
			}
		}
	} );
} );
