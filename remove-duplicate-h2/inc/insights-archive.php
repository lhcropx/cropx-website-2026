 ...
 * @param bool     $hide_heading_text   True when the calling template already
 *                                       rendered this same $heading as an H1 in
 *                                       a hero just above (cropx_render_archive_group_hero()) —
 *                                       repeating it as an H2 here would be a
 *                                       redundant duplicate (Aug 2026, Ag
 *                                       Insights). Only matters when there's no
 *                                       pill row to fill this slot instead —
 *                                       see $is_tags_only below. Pass false
 *                                       (default) for views with no hero of
 *                                       their own, e.g. tag.php and ungrouped
 *                                       categories, where this heading is the
 *                                       only place its text appears.
 */
function cropx_render_insights_grid( WP_Query $query, array $query_category_ids, array $badge_category_ids, array $accent_ids, array $pills, string $heading, string $intro, array $tags = array(), bool $show_badges = true, string $filter_param = 'categories', bool $hide_heading_text = false ): void {
		...
		<div class="wrap">
			<main id="main-content">
				...
				<section class="agr-grid-section" aria-label="<?php esc_attr_e( 'Articles', 'cropx' ); ?>">
					...
					// Whether the heading/intro text renders in place of pills.
					// When it's suppressed (a hero above already showed this
					// same heading, e.g. Ag Insights) AND there are no pills or
					// topic tags to fill the slot instead, skip the wrapper
					// entirely rather than leaving an empty div with its
					// bottom margin creating a blank gap above the grid.
					$show_heading_text = ! $is_tags_only && ! $hide_heading_text;
					$header_has_content = $show_tags_block || ! empty( $pills ) || $show_heading_text;
					?>
					<?php if ( $header_has_content ) : ?>
					<div class="agr-grid-header<?php echo $is_tags_only ? ' agr-grid-header--tags-only' : ''; ?>">

						<?php if ( $show_heading_text ) : ?>
