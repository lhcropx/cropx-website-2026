...
if ( $group_slug ) {
	...
	// This category belongs to a registered group, so cropx_render_archive_group_hero()
	// just rendered that group's H1 above. Matters most for a single-category
	// group's own top-level category page (e.g. /category/ag-insights/,
	// where $heading below is literally "Ag Insights" again — the exact
	// duplicate-H2 bug fixed on page-insights.php, Aug 2026); for a group with
	// real children (News's /category/press-releases/, etc.) $heading is a
	// different string, but $pills already fills this slot there regardless,
	// so this flag has no visible effect — set for consistency and in case
	// that ever changes.
	$hide_heading_text = true;
	...
	// Ungrouped category — no hero was rendered above, so this heading is the
	// only place its text appears. Keep it.
	$hide_heading_text = false;
...
global $wp_query;
cropx_render_insights_grid( $wp_query, $query_category_ids, $badge_ids, $accent_ids, $pills, $heading, $intro, $tags, $show_badges, 'categories', $hide_heading_text );
